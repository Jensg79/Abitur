...
\hspace*{1.5em}\(\rightarrow\) Praktisch am TR: \(\text{Lower}=-1\text{E}99\) (oder ähnlich extrem negativ).
\item \(\;P(X\ge x)\): Setze \(\text{Lower}=x\) und \(\text{Upper}\) ``sehr groß''.\\
\hspace*{1.5em}\(\rightarrow\) Praktisch am TR: \(\text{Upper}=1\text{E}99\) (oder ähnlich extrem positiv).
...
\end{b8d}
\subsection{Erkennungsmerkmale eine Normalverteilung}
%%%%%
...
\vspace{0.8em}
\textbf{Entscheidungsbaum}\index{Zufallsgröße!Stetige!Entscheidungsbaum}\newline
...
\begin{tikzpicture}[
  ...
  box/.style={
    ...
    inner sep=4pt, text width=0.55\linewidth
  },
  decision/.style={
  ...
  every node/.style={font=\small} % optional: nur leicht kleiner
]
