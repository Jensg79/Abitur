...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -3.5, xmax = 4.5, ymin= -4, ymax= 4,
        axis lines = middle, 
         x=1cm,y=1cm,
        ymajorgrids=true,
        xmajorgrids=true,
...
% --- Der Hinweis-Kasten ---
\node[
  name=warnbox,            % <- Name, damit wir die Ecken referenzieren können
  anchor=north east,
  rotate=-20,
  transform shape,
  inner sep=0pt
] at ([xshift=-0.5cm,yshift=6cm]current page.south east) {%
  \begin{achtung}[width=5cm,height=3cm]
Die mittlere Änderungsrate am Beispiel.
  \end{achtung}
};
\end{tikzpicture}
\end{subsection}
...
\end{merke*}
\begin{defi}{Differentialquotient oder Ableitung}{} \index{Ableitung!Definition}
...
Gegeben ist der Graph $G_f$ einer Funktion $f$ und die beiden beliebigen Punkte $P(x_0|f(x_0))$ und $Q(x_0+h|f(x_0+h))$. Durch $P$ und $Q$ verläuft eine Sekante am Graphen $G_f$. Der Abstand der beiden Punkte läuft bei der h-Methode gegen Null. 
  \begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -0.2, xmax = 3.5, ymin= -1.5, ymax= 5.5,
 x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
        ...
        \draw (1,1)-- ++(-2.5pt,-2.5pt) -- ++(5pt,5pt) ++(-5pt,0pt) -- ++(5pt,-5pt);
         ...
         \draw[red] (1.4,-0.3) node[right] {$\longleftarrow$};
         \draw[red] (1.3,-0.8) node[right] {$h\longrightarrow 0$};
\end{axis}
\end{tikzpicture}
\end{center}
 Das bedeutet, dass sich die beiden Punkten der Sekante beliebig genau annähern, ihr Abstand geht damit gegen Null ($h\rightarrow 0$). Somit wird aus der Sekante eine Tangente.
\end{bsp}
\begin{tikzpicture}[remember picture, overlay]
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=14cm]current page.south east) {%
\begin{achtung}[width=5cm,height=3cm]
Die graphische Darstellung der h-Methode an einem Beispiel!
\end{achtung}
};
\end{tikzpicture}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -2, xmax = 3.5, ymin= -1.5, ymax= 3.5,
 x=1cm,y=1cm,
        axis lines = middle, 
        xticklabels=\empty,
        yticklabels=\empty,
...
\begin{enumerate}
    ...
    \item Berechnung der Steigung an einer bestimmten Stelle $x_0$
...
\begin{merke}{Stetigkeit}{}
    Eine Funktion \(f\) ist stetig, genau dann wenn der Graph \(G_f\) der Funktion \(f\) in \(\mathds{D}_f\) ohne \enquote{absetzen} gezeichnet werden kann.
\end{merke}
\begin{bsp}{Die stetige Funktion \(f(x) = \dfrac{1}{x}\)}{}\index{Stetigkeit}\index{Differenzieren!Stetigkeit}
Die Funktion \(f(x)= \dfrac{1}{x}\) ist in ganz \(\mathds{D}_f = \mathds{R}\setminus\{0\} \) stetig. Man kann den Graphen \(G_f\) ohne abzusetzen zeichnen, da \(f\) nicht in \(x=0\) definiert ist.
\begin{center}
   \begin{tikzpicture}
\begin{axis}[
    axis lines=middle,
...
% rechter Ast (separat, damit x=0 ausgelassen wird)
\addplot[thick, domain=0.2:5] {1/x};

% (optional) Asymptoten andeuten
\addplot[dashed, very thick, red] coordinates {(0,-5) (0,5)};
\addplot[dashed, very thick, blue] coordinates {(-5,0) (5,0)};
\end{axis}
\end{tikzpicture} 
\end{center}
\end{bsp}
\begin{defi}{\(\epsilon - \delta\) Definition der Stetigkeit}{}\index{Stetigkeit!Nachweis}\index{Stetigkeit!Definition}
    Eine reelle Funktion \(f\) mit \(\mathds{D}_f \subseteq \mathds{R}\) und \(\mathds{W}_f \subseteq \mathds{R} \) ist stetig in \(x_0 \in \mathds{D}_f\) wenn es zu jedem \(\epsilon > 0\) ein \(\delta > 0\) existiert, so dass für alle \(x\in \mathds{D}_f\) mit \(\left|x-x_0 \right|<\delta\) gilt \(\left|f(x)-f(x_0)\right|<\epsilon\)
\end{defi}
\begin{merke}{Folgerung}{}
    Mathematisch kann die Stetigkeit nur an einzelnen Stellen überprüft werden. Der Graph von \(f\) ist in \(x_0 \in \mathds{D}_f\) stetig, wenn \[\lim\limits_{x\rightarrow x_0^+} f(x) =\lim\limits_{x\rightarrow x_0^-} f(x) \] gilt. Also bei der links- bzw. rechtsseitigen Annäherung an de Punkt \(x_0\) der gleiche Grenzwert auftritt.
\end{merke}
\begin{merke}{Sprungstellen}{}
    Macht der Graph einer Funktion \(h\) an einer Stelle der Definitionsbereichs einen \enquote{\textcolor{red}{Sprung}} so nennt man die Funktion an dieser Stelle unstetig und die Stelle wird als \textcolor{red}{Sprungstelle} bezeichnet.
\end{merke}
...
\end{bsp*}
\begin{b8d}{Zusammenhang der Differenzierbarkeit und der Stetigkeit}{}
Wenn die Funktion \(f\) in \(x_0\) differenzierbar ist, dann ist \(f\) in \(x_0\) auch stetig. Allerdings garantiert die Stetigkeit einer Funktion \(h\) in \(x_0\) nicht deren Differenzierbarkeit in \(x_0\). \\
Es gibt Funktionen, die überall stetig sind und nirgends differenzierbar\footnote{Siehe die Weierstraß-Funktion.}.
\end{b8d}
...
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},
x=1cm,y=1cm,
axis lines=middle,
ymajorgrids=true,
xmajorgrids=true,
...
\draw[color=red] (-2.2,3.5) node {$f'(x)$};
\end{scriptsize}
\end{axis}
\end{tikzpicture}
\end{center}
...
Die Potenzfunktion $f$ mit $f(x) = x^n$ und natürlichen Exponenten $n$ ist differenzierbar und es gilt: \[\highlightmath[red!65]{f'(x) = n\cdot x^{n-1}}\]
\underline{Merkregel:} Der Term $x^n$ wird differenziert, indem man den Exponenten $n$ als Faktor voranstellt und dann den Exponenten um 1 verringert.
\end{merke*}
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=-4cm]current page.north east) {%
\begin{achtung}[width=5cm,height=2.5cm]
Ableitungsregeln müssen sitzen! Siehe für alle Regeln \ref{AbleitungAllgemein}!
\end{achtung}
};
\end{tikzpicture}
...
\index{Newton-Verfahren!Taschenrechner}
\begin{b8d}{Newton-Verfahren (Nullstelle) am Casio fx-87DE Plus}{}
\begin{center}
\begin{tabular}{p{0.94\linewidth}}
\hline
\textbf{Route zum Newton-Verfahren (manuelle Iteration)}\\[4pt]
1) \taste{MODE} \;\(\rightarrow\)\; \taste{1} \;\(\rightarrow\)\; \textbf{COMP}\\
2) \textbf{Startwert speichern:} \(x_0\) eintippen \;\(\rightarrow\)\; \taste{SHIFT}\,\taste{STO}\,\taste{X}\\
\quad(\textit{damit ist die Variable \(X\) mit \(x_0\) belegt})\\[6pt]

\textbf{Newton-Term eingeben:}\\
Gib den Iterationsterm
\[
x_{n+1}=x_n-\frac{f(x_n)}{f'(x_n)}
\]
als \textbf{Rechenausdruck mit der Variablen \(X\)} ein, also:
\[
X-\frac{f(X)}{f'(X)}.
\]
(\textit{Für \(X\) nutzt du die Taste \taste{ALPHA}\,\taste{X}.})\\[6pt]

\textbf{Iterieren (Schleife):}\\
3) \taste{=} \;\(\rightarrow\)\; Ergebnis ist \(x_1\)\\
4) \taste{SHIFT}\,\taste{STO}\,\taste{X} \;\(\rightarrow\)\; speichert \(x_1\) in \(X\)\\
5) Mit \taste{\(\uparrow\)} die letzte Eingabe (Newton-Term) wieder aufrufen\\
6) \taste{\(=\)} \;\(\rightarrow\)\; berechnet \(x_2\)\\
7) Wiederhole Schritte 4)--6) für \(x_3,x_4,\dots\)\\[8pt]

\textbf{Abbruchkriterium (praxisnah):}\\
Stoppe, wenn zwei aufeinanderfolgende Werte praktisch gleich sind, z.\,B. \\
\(\lvert x_{n+1}-x_n\rvert < 10^{-3}\) (oder strenger).\\[6pt]

\textbf{Kontrolle:}\\
Am Ende zur Probe \(f(X)\) ausrechnen: ist \(\lvert f(X)\rvert\) klein, passt die Nullstelle.\\
\hline
\end{tabular}
\end{center}

\noindent
\textbf{Mini-Beispiel (zum schnellen Nachbauen):}
\(f(x)=x^2-2,\; f'(x)=2x\). Newton-Term:
\[
X-\frac{X^2-2}{2X}.
\]
Start z.\,B. \(x_0=1\) in \(X\) speichern und dann iterieren.\\[4pt]

\noindent
\textbf{Wichtige Hinweise (damit Newton nicht entgleist):}
\begin{itemize}
\item Wenn \(f'(x_n)=0\) (oder sehr klein), kann der Schritt riesig werden \(\rightarrow\) Verfahren kippt weg.
\item Startwert zählt: nah an der Nullstelle \(\Rightarrow\) meist sehr schnelle Konvergenz.
\item Wenn die Werte springen oder zyklisch werden: anderen Startwert wählen.
\end{itemize}
\end{b8d}
\begin{tikzpicture}[remember picture, overlay]
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=-6cm]current page.north east) {%
\begin{achtung}[width=5cm,height=2.5cm]
Alternativ kann man den Taschenrechner auch mit \taste{ANS} programmieren.
\end{achtung}
};
\end{tikzpicture}
\end{section}
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=-3cm]current page.north east) {%
\begin{achtung}[width=5cm,height=2.5cm]
Kurvendiskussion deckt \(\approx 50 \%\) des Abiturs ab!
\end{achtung}
};
\end{tikzpicture}
...
\end{equation*} 
 \end{defi}
 \begin{defi}{Extrema}{}
   Es sei $f$ eine auf einem Intervall $I$ definierte Funktion. Dann nennt man $f(x_0)$ \textcolor{red}{lokales Maximum} wenn es eine Umgebung $U$ um $x_0$ gibt, sodass für alle  $x \in U \in \mathds{D}_f$ gilt: \textcolor{red}{$f(x_0)\geq f(x)$}. Der Punkt \textcolor{red}{$P(x_0|f(x_0))$} heißt dann \textcolor{red}{Hochpunkt (HoP)}.\\
   Es sei $f$ eine auf einem Intervall $I$ definierte Funktion. Dann nennt man $f(x_0)$ \textcolor{red}{lokales Minimum} wenn es eine Umgebung $U$ um $x_0$ gibt, sodass für alle  $x \in U \in \mathds{D}_f$ gilt: \textcolor{red}{$f(x_0)\leq f(x)$}. Der Punkt \textcolor{red}{$P(x_0|f(x_0))$} heißt dann \textcolor{red}{Tiefpunkt (TiP)}.
 \end{defi}
 \begin{defi}{Terrassenpunkt}{}\index{Terrassenpunkt}
     Eine Funktion \(f\) hat in \(x_0\) einen \textbf{Terrassenpunkt}, wenn
\begin{itemize}
\item \(f\) in einer Umgebung von \(x_0\) mindestens zweimal differenzierbar ist,
\item die Tangente dort waagrecht ist: \(\;f'(x_0)=0\),
\item und \(x_0\) ein \textbf{Wendepunkt} ist, d.\,h. die Krümmung wechselt das Vorzeichen
      (äquivalent: \(f''\) wechselt in \(x_0\) das Vorzeichen).
\end{itemize}
Kurz: \(\;f'(x_0)=0\) und \(f\) hat in \(x_0\) einen Wendepunkt.
 \end{defi}
...
\end{enumerate}
 \end{itemize}
 \end{b8d}
  \begin{satz}{Kriterium für einen Terrassenpunkt}{}
Sei \(f\) dreimal differenzierbar in einer Umgebung von \(x_0\).
Gilt
\[
f'(x_0)=0,\qquad f''(x_0)=0,\qquad \text{und}\qquad f'''(x_0)\neq 0,
\]
dann besitzt \(f\) in \(x_0\) einen \textbf{Terrassenpunkt} (stationären Wendepunkt).
\end{satz}
 ...
 Vorgehen zum bestimmen der Monotonie und der Extrema:
 \begin{enumerate}
 ...
 \begin{center}$\begin{aligned}
    ...
    x_1&= \dfrac{-3+ 2\sqrt{3}}{3}\approx 0,15\\
    x_2&= \dfrac{-3 - 2\sqrt{3}}{3}\approx -2.15
	\end{aligned}$\end{center}
...
\end{tabular}
\end{center}
Berechnung der Lage der Extrempunkte:\\
\begin{center}$\begin{aligned}
	f(-2,15)&= 2 \cdot(-2,15)^3 +6\cdot (-2,15)^2 -2\cdot (-2,15)+1\\
 &\approx 13,16 \longrightarrow HoP(-2,15|13,16)
	\end{aligned}$\end{center}
 \begin{center}$\begin{aligned}
	f(0,15)&= 2\cdot(0,15)^3 +6\cdot (0,15)^2 -2\cdot (0,15)+1\\
 &\approx 0,84 \longrightarrow TiP(0,15|0,84)
	\end{aligned}$\end{center}
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -4.1, xmax = 2.5, ymin= -15.1, ymax= 20.5,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
        xtick={-4, -3,  ..., 2},
...
\end{center}
\end{bsp}
\begin{b8d}{Zusammenhang Graph der 1.Ableitung und Graph der Funktion}{}
 \begin{tikzpicture}[scale = 0.8]
        ...
        xlabel = $x$,
        ylabel=$y$]
            \addplot[color= red, samples = 300, domain= -3.7:1.7]{6*x^2+ 12*x -2};
    \addplot[color= black, samples = 300, domain= -3.7:1.7]{2*x^3+ 6*x^2 -2*x+5};
 \draw[red, dashed] (-2.15,0 )-- (-2.15,17.16);   
  \draw[red, dashed] (0.15,0 )-- (0.15,4.85);
        \end{axis}
    \end{tikzpicture}
     \begin{tikzpicture}[scale=0.8]
        \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -1.6, xmax = 4.1, ymin= -15.1, ymax= 10.5,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
        xtick={-1,  ..., 4},
        ...
        xlabel = $x$,
        ylabel=$y$]
            \addplot[color= red, samples = 300, domain= -1:3]{-1*6*x^2+ 12*x -2};
             \addplot[color= black, samples = 300, domain= -1:4]{-1*2*x^3+ 6*x^2 -2*x+1};
        \end{axis}
    \end{tikzpicture}
\end{b8d}
\begin{tikzpicture}[remember picture, overlay]
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=-5cm]current page.north east) {%
\begin{achtung}[width=4cm,height=5cm]
Graphisches differenzieren! Aufgabenstellung: Ordnen Sie dem Graphen der\\ Funktion den Graphen der Ableitung zu.
\end{achtung}
};
\end{tikzpicture}
\AddToHookNext{shipout/foreground}{%
  \begin{tikzpicture}[remember picture, overlay]
    \node[
      ...
      transform shape,
      inner sep=0pt
    ] at ([xshift=-0.25cm,yshift=5cm]current page.south east) {%
      \begin{achtung}[width=3.5cm,height=4cm]
     Ausführliche \\ Monotonietabelle. Meistens reicht eine Vorzeichenuntersuchung der 1.Ableitung.
      \end{achtung}
    };
  \end{tikzpicture}%
}
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -1.1, xmax = 5.5, ymin= -4.1, ymax=3.5,
         x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -2.1, xmax = 4.1, ymin= -6.1, ymax=6.1,
     x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{merke}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[
      ...
      transform shape,
      inner sep=0pt
    ] at ([xshift=-1.5cm,yshift=-5cm]current page.north east) {%
      \begin{achtung}[width=5cm,height=4cm]
    Bei Leistungserhebungen müssen die einzelnen Schritte\\ nicht auswendig gelernt werden. Es wird immer angegeben, was gemacht werden soll.
      \end{achtung}
    };
  \end{tikzpicture}%
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -5.1, xmax = 3.1, ymin= -3.1, ymax=6.1,
     x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
transform shape,
inner sep=0pt
] at ([xshift=-0.5cm,yshift=-15cm]current page.north east) {%
\begin{achtung}[width=5cm,height=3cm]
Integrationsregeln müssen sitzen! Siehe für alle Regeln \ref{InteRegel}!
\end{achtung}
};
\end{tikzpicture}
...
Bei einer Funktionenschar gibt es neben der Variable $x$ auch noch einen Parameter (häufig a oder k), welchen man frei auf eine Zahl festlegen kann. Für jede Belegung des Parameters bekommt man einen anderen Funktionsterm und somit auch einen anderen Funktionsgraphen.
\end{defi}
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -3.1, xmax = 3.1, ymin= -4.1, ymax=4.1,
     x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\begin{itemize}
    \item Bestimmung der Nullstellen in Abhängigkeit von a:
\begin{equation*}
    \begin{split}
        ...
        \end{split}
\end{equation*}
An dieser Stelle muss eine \textcolor{red}{Fallunterscheidung} erfolgen. Hierbei wird untersucht, für welche Werte von $a$ der Wurzelterm keine, eine bzw. 2 Lösungen besitzt.
...
\begin{enumerate}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[
      ...
      transform shape,
      inner sep=0pt
    ] at ([xshift=-0.25cm,yshift=8cm]current page.south east) {%
      \begin{achtung}[width=3.5cm,height=4cm]
     In vielen Fällen muss eine Fallunterscheidung\\ getroffen werden.
      \end{achtung}
    };
  \end{tikzpicture}%
}
...
\item Zwei Lösungen genau dann wenn der Parameter folgende Ungleichung erfüllt: $a_1<a<a_2$. Damit hat $f_a(x)$ genau zwei Nullstellen.
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -4.1, xmax = 4.1, ymin= -2.1, ymax=2.1,
     x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{center}
\end{bsp}
\subsection{Die Ortskurve}\index{Funktionen!Funktionenscharen!Ortskurve}
Die Ortskurve ist eine Kurve, auf der besondere Punkte einen Funktionenschar liegen. Alle diese Punkte haben eine gemeinsame Eigenschaft. Die Ortskurve wird häufig für die Extrempunkte bzw. die Wendepunkte erzeugt.
\begin{merke}{Bestimmung der Ortskurve besonderer Punkte}{}
    Um die Ortskurve von besonderen Punkten einer Funktionsschar zu bestimmen geht man wie folgt vor:
    \begin{enumerate}
        \item Man bestimmt die gesuchten Punkte (Scheitelpunkte, Extrema, Wendepunkte) in Abhängigkeit des Parameters.
        \item Auflösen der Stelle nach dem Parameter. 
        \item Ersetzen des Parameters in der Ausgangsgleichung.
    \end{enumerate}
    
\end{merke}
\begin{bsp}{Die Ortskurve}{}
Gegeben ist die Funktionsschar $f_t(x) = tx^4 - 4t^2x^2$ mit $\mathds{D} = \mathds{R}$ und $t > 0$. Bestimmung der Ortskurve aller Tiefpunkte\\
Berechnung der Extremstellen:
\begin{enumerate}
    \item Bestimmung der besonderen Punkte. In diesem Fall die Extremstellen des Graphen der Funktion $f_t(x)$
\begin{equation*}
    \begin{split}
     f_t(x) &= t\cdot x^4 - 4t^2\cdot x^2\\
     f'_t(x) &= 4t\cdot x^3 -8t^2\cdot x = 4x\cdot(tx^2-2t^2)\\
       0&= 4x\cdot(tx^2-2t^2)\\
       x_1 &= 0\\
       0 &= tx^2 -2t^2 \hspace{0.5cm} | + (2t^2)\\
       tx^2 &= 2t^2 \hspace{0.5cm} | :t \\
       x^2 &= 2t\\
       x_{2/3} &= \pm \sqrt{2t} 
       \end{split}
\end{equation*}
    \item Nachweis, dass es Extremstellen sind und Nachweis der Art der Extremstellen mit der zweiten Ableitung. 
    \begin{equation*}
    \begin{split}
     f_t(x) &= t\cdot x^4 - 4t^2\cdot x^2\\
     f'_t(x) &= 4t\cdot x^3 -8t^2\cdot x \\
       f''_t(x)&= 12t\cdot x^2 -8t^2\\
       f''_t(0) &= 12t \cdot (0)^2 -8t^2 = -8t^2 < 0 \Rightarrow Hochpunkt\\
       f''_t(\sqrt{2t}) &= 12t \cdot (\sqrt{2t})^2 -8t^2 = 24t^2 -8t^2 = 16t^2 >0 \Rightarrow Tiefpunkt \\
       f''_t(-\sqrt{2t}) &= 12t \cdot (-\sqrt{2t})^2 -8t^2 = 24t^2 -8t^2 = 16t^2 > 0  \Rightarrow Tiefpunkt
       \end{split}
\end{equation*}
Die Tiefpunkte liegen bei $x = \pm \sqrt{2t}$ bzw. bei $x^2 = 2t$
\item Umformung der Tiefpunkte nach dem Parameter t.
\begin{equation*}
    \begin{split}
     x^2 &= 2t \hspace{0.5cm} | :2\\
     t &= \dfrac{1}{2} x^2
       \end{split}
\end{equation*}
\item Ersetzen des Parameters $t=\dfrac{1}{2} x^2$ in der Ausgangsgleichung der Funktion.  
\begin{equation*}
    \begin{split}
    f_t(x) &= t\cdot x^4 - 4t^2\cdot x^2\\
     y &= \dfrac{1}{2} x^2 \cdot x^4 -4 \cdot(\dfrac{1}{2} x^2)^2 x^2\\
     &= \dfrac{1}{2} x^6 - x^6 = -\dfrac{1}{2} x^6
       \end{split}
\end{equation*}
Auf dem Graphen der Gleichung $y= -\dfrac{1}{2} x^6$ liegen damit alle Tiefpunkte der Funktion $f_t(x) = t\cdot x^4 - 4t^2\cdot x^2$.
\end{enumerate}
In folgendem Koordinatensystem ist die Ortskurve $y= -\dfrac{1}{2} x^6$ für die Funktionsschar\\ $f_t(x) = t \cdot x^4 -4t^2 \cdot x^2$.
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -3.1, xmax = 3.1, ymin= -5.1, ymax=2.1,
     x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
        ...
        \addplot[color= red, samples = 300, domain= -2:2]{-1/2 * x^6};
        \addplot[color= blue, samples = 300, domain= -2:2]{1/2 * x^4 -x^2};
        \addplot[color= green, samples = 300, domain= -2:2]{3/4 * x^4 -9/4*x^2};
        \addplot[color= orange, samples = 300, domain= -2:2]{9/10 * x^4 -81/25 * x^2};
        \addplot[color= black, samples = 300, domain= -2:2]{x^4 - 4*x^2};
        \draw[red] (0.9,0.5)   node [above] {$y= -\dfrac{1}{2} x^6$};
...
Eine Funktion $f$ mit $x\longrightarrow \dfrac{z(x)}{n(x)}$ nennt man gebrochen-rationale Funktion genau dann, wenn die Funktionen $z(x)$ und $n(x)$ Polynome sind. Die Funktion $f$ lässt sich damit durch folgenden Term beschreiben:$$f(x) = \dfrac{a_mx^m + a_{m-1}x^{m-1} + \ldots + a_1 x +a_0}{b_n x^n + b_{n-1}x^{n-1} + \ldots + b_1 x + b_0}$$ mit $m,n \in \mathds{N}$ und $a_m , b_n \neq 0$.
 \end{defi}
 \begin{bsp}{Beispiel für eine gebrochen-rationale Funktion}{} 
Der Graph der gebrochen-rationale Funktion  $$f(x)= \dfrac{x^2+4x+3}{x^2+x-2}$$ mit $\mathds{D}_f = \mathds{R}\setminus \{-2;1\}$. \\
Die Nullstellen des Nennerpolynoms ergeben die Definitionslücken und  damit folgt für den Graphen der Funktion folgendes Aussehen.
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},domain=-6:5, restrict y to domain=-5:5,axis x line=center, axis y line=center,%
     x=1cm,y=1cm,
xtick={-6,...,5}, ytick={-5,...,5}]
        \addplot[mark=none, color=red]  plot[samples=800,smooth]{((x+1)*(x+3)) / (x^2 +x - 2)};
        \draw[red] (3.4,1)   node [above] {$G_{f}$};
        \draw[color = blue] (-2,-5) -- (-2,5);
        \draw[color = blue] (1,-5) -- (1,5);
    \end{axis}
\end{tikzpicture}
\end{center}
\end{bsp}
...
\end{bsp}
  \begin{tikzpicture}[remember picture, overlay]
    \node[
      ...
      transform shape,
      inner sep=0pt
    ] at ([xshift=-1cm,yshift=12cm]current page.south east) {%
      \begin{achtung}[width=3.5cm,height=4cm]
     Die Polynomdivision ist\\ allerdings nicht mehr Teil des Lehrplans.
      \end{achtung}
    };
  \end{tikzpicture}%

 \subsubsection{Grenzwertbetrachtungen bei gebrochen-rationalen Funktionen} \index{Funktionen!Gebrochen-rationale!Grenzwertbetrachtungen} 
 \begin{merke}{Untersuchung der Definitionslücke}{}
     Eine rationale Funktion \textcolor{red}{\textbf{kann}} Definitionslücken aufweisen. Diese sind dann die Nullstellen des Nenners.\\
     Die Definitionslücken lassen sich in zwei Arten einteilen:
     \begin{enumerate}
         \item Die Definitionslücken die durch \textcolor{red}{\textbf{kürzen behoben}} werden können. In diesem Fall hat der Graph an der Definitionslücke ein Lücke im Graphen.      
         \item Streben die Funktionswerte bei einer Annäherung an die Definitionslücke entweder gegen $+\infty$ oder gegen $-\infty$ so nennt man die Definitionslücke Pol. Wechseln die Funktionswerte $f(x)$ bei Annäherung von links und von rechts an den Pol das Vorzeichen, liegt ein Pol mit Vorzeichenwechsel (VZW) vor, anderfalls ist es ein Pol ohne Vorzweichenwechsel. Hat die Funktion einen Pol so hat der Graph der Funktion an dieser Stelle eine senkrechte Asymptote.
     \end{enumerate}
 \end{merke}
 \begin{bsp}{Untersuchung auf Polstellen}{}
 \begin{enumerate}
    \item Gegeben ist die Funktion f mit $g(x) = \dfrac{(x+1)(x-1)}{x-1}$ und $\mathds{D}_g = \mathds{R}\setminus\{1\}$. Durch eine Termumformung ändert sich der Funktionsterm wie folgt: $$g(x) = \dfrac{(x-1)(x+1)}{(x-1)} =\dfrac{\cancel{(x-1)}(x+1)}{\cancel{(x-1)}} = x+1. $$ Der Graph der Funktion $g$ ist damit eine Gerade mit der Definitionslücke bei $x=1$.
    \begin{center}
\begin{tikzpicture}
    \begin{axis}
[restrict y to domain=-3:4, axis x line=center, axis y line=center,%
 x=1cm,y=1cm,
    xmin= -2.1, xmax = 3.1,
    ymin= -1.6, ymax=4.1,
        axis lines = middle, 
        ...
        xlabel = $x$,
        ylabel=$y$]
       \draw (1,2) circle[radius=0.08];
        \addplot[color= black, samples = 300, domain= -3:0.95,line width=1pt]{x+1};
        \addplot[color= black, samples = 300, domain= 1.05:4,line width=1pt]{x+1};     
        \draw (-1,0.5)   node [above] {$g(x)$};
        \draw (1.35,1.25)   node [above] {Def-lücke};
    \end{axis}
\end{tikzpicture}
\end{center}

     \item Gegeben ist die Funktion f mit $f(x) = \dfrac{(x-1)}{(x-1)(x+2)}$ mit $\mathds{D}_f = \mathds{R}\setminus\{-2, 1\}$. Durch Termumformungen ist es möglich, die Definitionslücke bei $x=1$ zu heben. Dadurch ändert sich zwar der Term allerdings bleiben die Definitionslücke bei $x= 1$ und $x=-2$ erhalten. $$f(x) = \dfrac{\cancel{(x-1)}}{\cancel{(x-1)}(x+2)} = \dfrac{1}{x+2}.$$
     \item Gegeben ist die Funktion h mit $h(x) = \dfrac{1}{x+2}$ mit $\mathds{D}_h = \mathds{R}\setminus\{-2\}$. Bei dieser Funktion kann die Definitionslücke nicht behoben werden. Um zu entscheiden, ob eine Polstelle mit oder ohne VZW vorliegt, muss eine Grenzwertbetrachtung an der Definitionslücke durchgeführt werden.
     \begin{equation*}
    \begin{split}
        \lim_{x\rightarrow -2^+} h(x) &= \lim_{x\rightarrow -2^+} \dfrac{1}{x+2} = \infty\\
        \lim_{x\rightarrow -2^-} h(x)&= \lim_{x\rightarrow -2^-} \dfrac{1}{x+2} = -\infty
    \end{split}
\end{equation*}
 Damit liegt eine Polstelle mit Vorzeichenwechsel vor.
 \begin{center}
\begin{tikzpicture}
    \begin{axis}
[restrict y to domain=-4:4, axis x line=center, axis y line=center,%
 x=1cm,y=1cm,
    ...
    ymin= -4.1, ymax=3.1,
        ...
        xlabel = $x$,
        ylabel=$y$]
        \addplot[color= black, samples = 300, domain= -7:-2,line width=1pt]{((1) / (x+2)};
        \addplot[color= black, samples = 300, domain= -2:3,line width=1pt]{((1) / (x+2)};
        \draw [ red,dashed,line width=1pt] (-2,-4) -- (-2,3);
        \draw (-3,-2)   node [above] {$h(x)$};
        \draw[red] (0,-1)   node [above] {senkrechte Asymptote};
        \draw[red] (0,-1.5)   node [above] {Polstelle mit VZW};
    \end{axis}
\end{tikzpicture}
\end{center}
 \end{enumerate}
 ...
 \begin{merke}{Schräge Asymptoten}{}
    Nähert sich der Graph einer rationalen Funktion $f$ für $x\longrightarrow \infty$ und für  $x\longrightarrow -\infty$ immer an eine Gerade $y=m\cdot x +t$ an, so nennt man diese Gerade eine Asymptote.
    \begin{enumerate}
        \item Für den Fall $m=0$ spricht man von einer waagerechten Asymptote.
        \item Für den Fall $m\neq 0$ spricht man von einer schrägen Asymptote.
    \end{enumerate} 
    Nähert sich der Graph einer rationalen Funktion $f$ für $x\longrightarrow \infty$ und für  $x\longrightarrow -\infty$ immer an eine Kurve beliebigen Grades an, so nennt man diese Kurve eine Näherungskurve.\\ 
\end{merke}
\begin{b8d}{Nachweis der schrägen Asymptote}{}
Um einen Nachweis zu führen, dass eine Gerade eine schräge Asymptote ist, muss die  Differenz \\ $(f(x) - (m\cdot x +t))$ für $x\longrightarrow \pm \infty$ gegen Null streben. $$\lim_{x\longrightarrow \pm \infty} (f(x) - (m\cdot x +t)) = 0$$
\end{b8d}
\begin{b8d}{Ablesen der schrägen Asymptote}{}
Liegt der Term der Funktion in der Summenform vor, ist es möglich die schräge Asymptote bzw. die waagerechte Asymptote direkt aus dem Term abzulesen. Hierbei ist der Teil des Terms ohne Nenner die schräge Asymptote bzw. die waagerechte Asymptote.
\end{b8d}
\begin{bsp}{}{}
\begin{itemize}
    \item $f(x) = \dfrac{x^3+x^2+1}{x^2} = \underbrace{x+1}_{\text{Gleichung der schrägen Asymptote}} +\dfrac{1}{x^2}$
    \item $f(x) = \dfrac{x+2}{x+1}= \dfrac{1}{x+1} + \underbrace{1}_{\text{Gleichung der waagerechten Asymptote}}$
\end{itemize}
\end{bsp}
Die Grenzwertbetrachtungen bei gebrochen-rationalen Funktionen kann über die Auswertung von Zähler- und Nennergrad erfolgen. Hierbei versteht man unter dem Grad eines Polynoms den höchsten vorkommenden Exponenten. Man unterscheidet dabei vier mögliche Fälle.

\begin{merke}{Fallunterscheidungen für die Betrachtung des Grenzwertverhalten}{}
...
\begin{enumerate}
    \item Grad des Zählerpolynoms  $z<n$ Grad des Nennerpolynoms\\
...
\item  Grad des Zählerpolynoms $z=n$ Grad des Nennerpolynoms\\ Bei Grenzwertuntersuchungen findet man eine waagerechte Asymptote die parallel zur x-Achse verläuft. Hierbei ergibt sich die waagerechte Asymptote aus dem Quotienten der Vorfaktoren der größten Exponenten.\\ Als Beispiel wird die Funktion $f(x)= \dfrac{2x^2}{3x^2-x+1}$, hierbei folgt aus der Grenzwertbetrachtung: $$\lim_{x\longrightarrow \infty} f(x) = \lim_{x\longrightarrow \infty} \dfrac{2x^2}{3x^2-x+1} = \dfrac{2}{3}$$ und $$\lim_{x\longrightarrow -\infty} f(x) = \lim_{x\longrightarrow -\infty} \dfrac{2x^2}{3x^2-x+1} = \dfrac{2}{3}$$
\item Grad des Zählerpolynoms $z=n+1$ Grad des Nennerpolynoms\\
...
\item Grad des Zählerpolynoms $z>n+1$ Grad des Nennerpolynoms +1\\
...
\end{enumerate}
\end{merke}
\begin{b8d}{Berechnung des Grenzwertes}{}
Um das Verhalten einer gebrochen-rationalen Funktion $f$ für $x \longrightarrow \pm \infty$ zu untersuchen, dividiert man in der Bruchform Zähler und Nenner durch die höchste Potenz
des Nennerpolynoms.\\
Liegt eine Summenform vor, eignet sich diese häufig für die Untersuchung des
genaueren Verhaltens von $f(x)$ für $x \longrightarrow \pm \infty $.
\end{b8d}
\subsubsection{Kurvendiskussion einer gebrochen-rationalen Funktion}
...
Während der Kurvendiskussion müssen die Definitionslücken beachtet werden. Das bedeutet, dass sowohl bei der Monotonietabelle als auch bei der Krümmungstabelle die Definitionslücke berücksichtigt werden müssen. An der Definitionslücke kann sich sowohl die Monotonie als auch die Krümmung ändern.
 \end{b8d}
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -4.1, xmax = 4.1, ymin= -4.1, ymax=6.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{itemize}
 ...
 \begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -1.1, xmax = 6.1, ymin= -1.1, ymax=4.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
Alle Punkte $P(x|y)$ die auf einem Kreis mit Radius $r$ um den Ursprung des Koordinatensystems liegen, erfüllen die Gleichung $$x^2+y^2 = r^2.$$ Bei der Bildung der Umkehrfunktion $f^{-1}$ folgt für die den Graphen der Umkehrfunktion $G_{f^{-1}}$ der Term $f^{-1}(x) = \sqrt{r^2-x^2}$. Für die Werte $-r\leq x\leq r$ ergibt sich als Graph ein Halbkreis mit Radius $r$ um den Ursprung.
 \begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -2.5, xmax = 2.5, ymin= -1.1, ymax=2.5,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{center}
 ...
 \begin{merke}{Halbkreise mit beliebigen Mittelpunkt}{}
   ...
   \begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -2.5, xmax = 5.5, ymin= -1.1, ymax=5.5,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
Die Ableitung einer Wurzelfunktion (Ableitungen der Wurzelfunktion findet man in \ref{Ableitungen}.) kann auf zwei unterschiedliche Arten erfolgen. 
Man verwendet entweder die Kettenregel mit der Wurzel als äußerer Funktion und dem Radikanden als innere Funktion oder wandelt die Wurzel in einen rationalen Exponenten um und wendet dann die Kettenregel an.
\begin{enumerate}
...
\begin{tikzpicture}[scale=1.5]
  ...
  every tick label/.append style={fill=none},
      axis lines = middle, 
      xlabel=$x$, 
      ylabel=$y$, 
...
]    \addplot[red,thick,samples=300,domain=-180:-90.1]{tan(x)};
\addplot[red,thick,samples=300,domain=-89.9: 89.9]{tan(x)};
\addplot[red,thick,samples=300,domain= 90.1:269.9]{tan(x)};
\addplot[red,thick,samples=300,domain=270.1:360]{tan(x)};
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -4.1, xmax = 2.6, ymin= -0.1, ymax=6.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -4.1, xmax = 4.6, ymin= -4.1, ymax=4.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\begin{enumerate}
    ...
    \hline
    \hline
    $G_f$ & smw & $HoP(-2|16\cdot e^{-2})$ & smf  & $TiP(0|0)$& smw\\
    \hline
\end{tabular}
\end{center}
...
Berechnung der Koordinaten der Wendepunkte:
        ...
        \begin{center}
    \begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -6.1, xmax = 1.1, ymin= -1.4, ymax=4.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
        ...
        \draw (0,0) -- ++(-2.5pt,-2.5pt) -- ++(5pt,5pt) ++(-5pt,0pt) -- ++(5pt,-5pt);
         \draw (0.3,-0.5)   node [above] {TiP};
         \draw (-2,2.17) -- ++(-2.5pt,-2.5pt) -- ++(5pt,5pt) ++(-5pt,0pt) -- ++(5pt,-5pt);
         \draw (-2,2.2)   node [above] {HoP};
...
Die natürliche Logarithmusfunktion $f(x) = \ln{(x)}$ ordnet jeder positiven reellen Zahl den natürlichen Logarithmus zu. Für die Funktion gilt damit: $$\mathds{D}_f = \mathds{R}^+$$ und $$\mathds{W}_f = \mathds{R}.$$
 ...
 \begin{center}
\begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -1.1, xmax = 4.6, ymin= -4.1, ymax=2.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{satz}
 ...
 Die Kurvendiskussion einer $\ln$-Funktion unterscheidet wird anhand folgendes Beispiel kurz erläutert.
...
\begin{center}
 \begin{tikzpicture}
    \begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},xmin= -1.1, xmax = 2.6, ymin= -1.1, ymax=5.1,
    x=1cm,y=1cm,
        axis lines = middle, 
        ymajorgrids=true,
        xmajorgrids=true,
...
\end{bsp}
\section{Flächeninhalt und bestimmtes Integral}\index{Integral}
    \subsection{Das bestimmte Integral}\index{Integral!Bestimmtes Integral}
       \begin{defi}{Das bestimmte Integral}{}
        Eine Funktion $f$ mit $f(x)\geq 0$ sei in einem Intervall $I = [a; b]$ mit $I \in \mathds{D}_f$ definiert. Der Inhalt der Fläche zwischen dem Graphen von $f$ und der $x-$Achse über dem Intervall $[a;b]$ ist der gemeinsame Grenzwert der Obersumme $O_n$ und der Untersumme $U_n$. Es gilt also $\lim\limits_{n \longrightarrow \infty} O_n = \lim\limits_{n \longrightarrow \infty} U_n $. Existiert dieser gemeinsame Grenzwert, dann nennt man diesen das bestimmte Integral der Funktion $f$ über dem Intervall $I = [a;b]$. Für das bestimmte Integral schreibt man dann $\integral[limits-mode=limits,limits={a, b}]{f(x)}$ und bezeichnet die Funktion $f$ integrierbar über $I=[a,b]$. \\
        Hierbei wird die zu integrierende Funktion $f$ als Integrandenfunktion und die durch den Parameter $dx$ gegebene Variable x als Integrationsvariable bezeichnet. Die Grenzen des Intervalls $I= [a,b]$ bilden die \textcolor{red}{unter-} bzw. die \textcolor{red}{obere} Grenze des Integrals.
    \end{defi}
    \begin{b8d}{Folgerungen aus der Definition}{}
    \begin{description}
    \item[i)] Fallen obere und untere Grenze zusammen, so ergibt sich für den Wert des Integrals \textcolor{red}{$\integral[limits-mode=limits,limits={a, a}]{f(x)} = 0$}
    \item[ii)] Den Wert des Integrals $\integral[limits-mode=limits,limits={a, c}]{f(x)}$ lässt sich Schrittweise angeben. Es gilt damit \\ \textcolor{red}{$\integral[limits-mode=limits,limits={a, c}]{f(x)} = \integral[limits-mode=limits,limits={a, b}]{f(x)} + \integral[limits-mode=limits,limits={b, c}] {f(x)}$ } für einen Wert $b$ mit \textcolor{red}{$a<b<c$}.
    \item[iii)] Vertauscht man die Grenzen des Integrals ändert sich das Vorzeichen.\\ \textcolor{red}{ $\integral[limits-mode=limits,limits={a, b}] {f(x)} = - \integral[limits-mode=limits,limits={b, a}] {f(x)}$}. \textcolor{red}{rückwärts Integration}
    \end{description}
    \end{b8d}
    \newpage
    \begin{bsp}{}{}
      \begin{tikzpicture}
\begin{groupplot}[
  group style={group size=3 by 1, horizontal sep=\hsep},
  width=\gpw, height=\gph,
  scale only axis=true,
  xmin=0, xmax=3.5, ymin=0, ymax=9.5,
  axis lines=left,
  xlabel={$x$}, ylabel={$y$},
...
\nextgroupplot
    ...
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2}{0}{3}{3}
    %\pgfplotsset{title={Untersumme $n=3$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}}
    %\addlegendentryexpanded{Untersumme $n=3$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};
\nextgroupplot
%%%%%%% Untersumme n=6
        \addlegendentry{$f(x)=x^2$}
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2}{0}{3}{6}
   % \pgfplotsset{title={Untersumme $n=6$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}}
    %\addlegendentryexpanded{Untersumme $n=6$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};
...
\nextgroupplot
    ...
    \addlegendentry{$f(x)=x^2$}
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2}{0}{3}{12}
   % \pgfplotsset{title={Untersumme $n=12$, $S_{12}=\pgfmathprintnumber{\lastRiemannSum}$}}
    %\addlegendentryexpanded{Untersumme $n=12$, $S_{12}=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};
  \end{groupplot}
\end{tikzpicture}\\
Als Beispiel die Obersummen der Funktion $f(x) =x^2$ im Intervall $I=[0,3]$. \\
 \begin{tikzpicture}
\begin{groupplot}[
  group style={group size=3 by 1, horizontal sep=\hsep},
  width=\gpw, height=\gph,
...
\nextgroupplot
    %%%%%% Untersumme n=3
    \addlegendentry{$f(x)=x^2$}
    \riemannbars[mode=upper, fill=red!35, draw=black]{\x^2}{0}{3}{3}
    %\pgfplotsset{title={Obersumme $n=3$, $S_3=\pgfmathprintnumber{\lastRiemannSum}$}}
    %\addlegendentryexpanded{Obersumme $n=3$, $S_3=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};

\nextgroupplot
%%%%%%% Untersumme n=6
        \addlegendentry{$f(x)=x^2$}
    \riemannbars[mode=upper, fill=red!35, draw=black]{\x^2}{0}{3}{6}
   % \pgfplotsset{title={Obersumme $n=6$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}}
   % \addlegendentryexpanded{Obersumme $n=6$, $S_6=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};
...
\nextgroupplot
    ...
    \addlegendentry{$f(x)=x^2$}
    \riemannbars[mode=upper, fill=red!35, draw=black]{\x^2}{0}{3}{12}
   % \pgfplotsset{title={Obersumme $n=12$, $S_{12}=\pgfmathprintnumber{\lastRiemannSum}$}}
    %\addlegendentryexpanded{Obersumme $n=12$, $S_{12}=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[very thick, domain=0:3, samples=200] {\x^2};
  \end{groupplot}
\end{tikzpicture}    
    \end{bsp}
...
\section{Hauptsatz der Differential- und Integralrechnung} 
   \begin{defi}{Die Flächenbilanz und das bestimmte Integral}{}\index{Integral!Flächenbilanz}
       \begin{multicols}{2} Gegeben ist eine Funktion f, die auf dem Intervall $I=[a,b]$ integrierbar ist. Das bestimmte Integral $\integral[limits-mode=limits,limits={a, b}]{f(x)}$ gibt die  \textcolor{red}{Flächenbilanz} der Fläche an, die der Graph von $f$ mit der x-Achse zwischen der unteren Grenze $a$ und der oberen Grenze $b$ einschließt. Dabei werden Flächeninhalte oberhalb der x-Achse positiv und Flächeninhalte unterhalb der x-Achse negativ gewertet.\\
       \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,line cap=round,line join=round]
  % ---------- Parameter ----------
  \def\xmin{-1.2} \def\xmax{7.3}
  \def\ymin{-2} \def\ymax{1.8}

  % einfache Cubic als Beispiel-Funktion:
  % Nullstellen bei x=-0.5, 2.2, 6.0  → + dann – wie im Bild
  \def\K{0.085}                         % Skalierung (Höhe der Wellen)
  \def\fx(#1){\K*(#1+0.5)*(#1-2.2)*(#1-6.0)}

  % Grenzen für die Schraffur
  \def\a{1.0}
  \def\xb{2.2}   % rechtes Ende der +Fläche: die Nullstelle
  \def\b{5.2}

  % Auswertungen für die roten Lotlinien
  \pgfmathsetmacro{\fa}{\fx(\a)}
  \pgfmathsetmacro{\fb}{\fx(\b)}

  % ---------- Achsen ----------
  \draw[thick,-Latex] (\xmin,0) -- (\xmax,0) node[right] {$x$};
  \draw[thick,-Latex] (0,\ymin) -- (0,\ymax) node[above] {$y$};
  \node[below left] at (0,0) {$0$};

  % ---------- positive Fläche: a → x_b ----------
  \fill[orange!70,opacity=.7]
    (\a,0)
    -- plot[domain=\a:\xb,samples=250,smooth] (\x,{ \fx(\x) })
    -- (\xb,0) -- cycle;

  % ---------- negative Fläche: x_b → b ----------
  \fill[blue!50,opacity=.25]
    (\xb,0)
    -- plot[domain=\xb:\b,samples=250,smooth] (\x,{ \fx(\x) })
    -- (\b,0) -- cycle;

  % ---------- Kurve G_f ----------
  \draw[very thick,blue!70!black]
    plot[domain=\xmin:\xmax -1,samples=500,smooth] (\x,{ \fx(\x) });

  % ---------- rote Lotlinien + Labels a,b ----------
  \draw[red!80,very thick] (\a,0) -- (\a,\fa) node[below=3pt,xshift=-5pt,black] {$\color{red}a$};
  \draw[red!80,very thick] (\b,0) -- (\b,\fb) node[below=3pt,black] {$\color{red}b$};

  % ---------- Zusatztexte ----------
  \node[blue!70!black] at (3.4,1.8) {$G_f$};
  \node at (1.4,0.25) {$+$};
  \node at (4.2,-0.5) {$-$};
\end{tikzpicture}
\end{multicols}
   \end{defi}
\begin{defi}{Die Integralfunktion}{}\index{Integral!Integralfunktion}
\begin{multicols}{2}
Gegeben sei eine in einem Intervall $I$ integrierbare Funktion 
$f: t \mapsto f(t)$. 
Dann heißt für jedes $a \in I$ die Funktion

  $I_a : x \mapsto 
  \integral[limits-mode=limits,
            limits={a, \textcolor{red}{x}},
            variable={t}]{f(\textcolor{blue}{t})},
  \qquad x \in I,$

die \textcolor{red}{Integralfunktion von $f$ zur unteren Grenze $a$}.


% ---- TikZ-Grafik separat ----
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex,line cap=round,line join=round, scale =1.5]
  % ---------- Parameter ----------
  \def\a{0.5}       % untere Grenze a
  \def\xvar{3.0}    % aktueller Wert von x (obere Grenze)
  \def\fx(#1){0.6+0.4*sin(60*(#1))}

  % ---------- Achsen ----------
  \draw[->,thick] (-0.3,0) -- (4.8,0) node[right] {$x$};
  \draw[->,thick] (0,-0.3) -- (0,1.7) node[above] {$y$};

  % ---------- Schattierte Fläche ----------
  \fill[blue!30]
    (\a,0)
    -- plot[domain=\a:\xvar,smooth,samples=200] (\x,{ \fx(\x) })
    -- (\xvar,0) -- cycle;

  % ---------- Funktionsgraph ----------
  \draw[very thick,blue!70!black]
    plot[domain=0:4.5,samples=300,smooth] (\x,{ \fx(\x) });

  % ---------- Vertikale Linien + Labels ----------
  \draw[dashed,red] (\xvar,0) -- (\xvar,{ \fx(\xvar) });
  \node[below,red] at (\xvar,0) {$x$};

  \draw[dashed,red] (\a,0) -- (\a,{ \fx(\a) });
  \node[below,red] at (\a,0) {$a$};

  % ---------- Text / Beschriftung ----------
  \node[blue!60!black] at (2.0,1.4) {$G_f$};
  \node at (2.2,-0.6) {\small\textcolor{blue!50!black}{$\displaystyle I_a(x)=\int_a^x f(t)\,dt$}};
\end{tikzpicture}
\end{center}
\end{multicols}
\end{defi}
\begin{b8d}{Nullstellen der Integralfunktion}{}\index{Integral!Integralfunktion!Nullstellen}
Aufgrund der Definition des bestimmten Integral ist $I_a(a) = \integral[limits-mode=limits,limits={a, a}, variable={t}]{f(t)} = 0$. Somit ist die untere Grenze $a$ stets eine Nullstelle von $I_a$. 
\end{b8d}
\begin{defi}{Hauptsatz der Differential- und Integralrechnung}{}\index{Integral!Hauptsatz Differential- und Integralfunktion}
\index{Ableitung!Hauptsatz Differential- und Integralfunktion}
    Die Funktion $f$ sei im Intervall $I=[r;s]$ stetig. Für die Integralfunktion $I_a : x \mapsto \integral[limits-mode=limits,
            limits={a, \textcolor{red}{x}},
            variable={t}]{f(\textcolor{blue}{t})}$ gilt dann $I'_a(x) = f(x)$ für $a,x \in [r,s]$.
\end{defi}
 \begin{b8d}{Folgerung aus dem Hauptsatz}{}
Die Integralfunktion $I_a$ der Funktion $f$ ist damit eine Stammfunktion von $f$.
 \end{b8d}
 
 \begin{satz}{Berechnung von Integralen}{}
     Die Funktion $f$ sei im Intervall $I$ mit  $I=[a, b]$ stetig. Ist $F$ eine \textcolor{red}{beliebige} Stammfunktion von $f$ in diesem Intervall, dann gilt
    \textcolor{red}{ $$\integral[limits-mode=limits,
            limits={a, b},
            variable={x}]{f(x)} = [F(x)]_a^b = F(b) - F(a)$$}
 \end{satz}
 \begin{b8d}{Erläuterung Integral}{}
Bei der Berechnung eines Integrals verwendet man folgende Schreibweise:\\
\begin{tikzpicture}[
  >=Latex,
  every node/.style={align=left}
]
  % Gleichung in Segmenten, damit man gezielt pfeilen kann
  \node (int)  {$\displaystyle \int_{a}^{b} f(x)\,dx$};
  \node (eq1)  [right=1.2em of int] {$=$};
  \node (br)   [right=0.8em of eq1] {$\displaystyle [F(x)]_{a}^{b}$};
  \node (eq2)  [right=0.8em of br] {$=$};
  \node (diff) [right=0.8em of eq2] {$\displaystyle F(b)-F(a)$};

  % Erklärtexte
  \node (txt1) [above=2.0em of br, anchor=south west]
     {\textcolor{red}{\textbf{eine}} Stammfunktion von \textcolor{blue}{$f$} in eckigen Klammern};
  \node (txt2) [below=2.0em of br, anchor=north west]
     {obere und untere Grenze};

  % Pfeile
  \draw[->] (txt1.south west) .. controls +(0,-1.2em) and +(0,1.0em) .. (br.north);
  % Zielpunkt für Grenzen (nahe Exponenten/Subskripten)
  \coordinate (limits) at ($(br.south east)+(-0.5em,0.2em)$);
  \draw[->] (txt2.north west) .. controls +(0,1.2em) and +(0,-1.0em) .. (limits);
\end{tikzpicture}
\end{b8d}
\begin{bem}{Zusammenhang zwischen Stammfunktion und Integralfunktion}{}\index{Integral!Zusammenfassungen}
\scalebox{0.65}{
    \begin{tikzpicture}[scale = 0.8]
% Farben/Strichstärken (bei Bedarf anpassen)
\tikzset{
  outer/.style = {draw=blue!70!black, line width=1.8pt},
  inner/.style = {draw=black!70, line width=1.2pt}
}

% Äußere Ellipse
\draw[outer, fill=blue!20, fill opacity=0.3] (0,0) ellipse (5.3cm and 4.3cm);

% Innere Ellipse (leicht nach rechts/unten versetzt)
\draw[inner,fill=orange!40, fill opacity=0.3] (1.8,-0.3) ellipse (3.2cm and 1.5cm);

% Beschriftungen (zentriert, Zeilenumbruch per text width)
\node[align=center,text width=9.8cm] at (0,2.1)
      {\bfseries Menge aller Stammfunktionen von $f$};

\node[align=center,text width=5.6cm] at (1.8,-0.5)
      {\bfseries Menge aller Integralfunktionen von $f$};

\end{tikzpicture}
\begin{tikzpicture}[scale = 1.5, >=Latex]

% Knoten für die Formeln
\node (fx) at (0,0) {\Huge{$f(x)$}};
\node (integral) at (6,0) {\Huge{$\displaystyle \int_a^x f(t)\,dt$}};

% oberer Bogen (Integration)
\draw[blue, very thick, ->]
  (fx.north east) .. controls (3,2.2) .. (integral.north west);
\node[blue, font=\bfseries, above] at (3,2.1) {von $a$ bis $x$ integrieren};

% unterer Bogen (Differenzieren)
\draw[red, very thick, <-]
  (fx.south east) .. controls (3,-2.0) .. (integral.south west);
\node[red, font=\bfseries, below] at (3,-2.0) {nach $x$ differenzieren};

\end{tikzpicture}}
\end{bem}
\section{Zusammenfassung bestimmtes Integral und Integralfunktion}
\begin{b8d}{\emph{„Wie kann man die Fläche unter einem Funktionsgraphen berechnen?“}}{}
\begin{itemize}
    \item Flächenberechnung mit Rechteckapproximation (Riemann-Summe).\\
...
\nextgroupplot
    ...
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2+1}{0}{3}{3}
    %\pgfplotsset{title={$U_3=\pgfmathprintnumber{\lastRiemannSum}$}}
\addlegendentryexpanded{$U_3=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[thick, domain=0:3, samples=200] {\x^2+1};

\nextgroupplot
%%%%%%% Untersumme n=6
        \addlegendentry{$f(x)=x^2+1$}
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2+1}{0}{3}{6}
   % \pgfplotsset{title={Untersumme $n=6$, $U_6=\pgfmathprintnumber{\lastRiemannSum}$}}
    \addlegendentryexpanded{$U_6=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[thick, domain=0:3, samples=200] {\x^2+1};
...
\nextgroupplot
    ...
    \addlegendentry{$f(x)=x^2+1$}
    \riemannbars[mode=lower, fill=red!35, draw=black]{\x^2+1}{0}{3}{12}
    %\pgfplotsset{title={Untersumme $n=12$, $U_{12}=\pgfmathprintnumber{\lastRiemannSum}$}}
    \addlegendentryexpanded{$U_{12}=\pgfmathprintnumber{\lastRiemannSum}$}
    \addplot[thick, domain=0:3, samples=200] {\x^2+1};
  \end{groupplot}
  
\end{tikzpicture}}
\begin{itemize}
    \item Streifen unter- bzw. oberhalb des eigentlichen Funktionsgraphen
    \item je schmaler die Streifen desto genauer die Annäherung an die eigentliche Fläche
    \item Für die Ober- und Untersumme gilt im Intervall $I=[a,b]$ damit \\ $\lim\limits_{n \longrightarrow \infty} O_n = \lim\limits_{n\longrightarrow \infty} U_n =\integral[limits-mode=limits,limits={a, b}]{f(x)}$
\end{itemize}
    \item Das bestimmte Integral als Flächenbilanz

    \begin{itemize}
        \item Für die Funktion $f(x) = x^2+1$ im Intervall $I=[0|3]$
  ...
  bound y shift=2pt,
    ...
    \scalebox{0.75}{\ShadeArea{\x^2+1}{0}{3}}\\
    \item Für die Funktion $f(x) = x^3-3x$ im Intervall $I=[-2,5|2,7]$\\
    \IntegralSetup{
  xmin=-4, xmax=2.3,
  ymin=-9.3, ymax=6.3,
  width=15cm, height=8cm,
  ...
  bound y shift=2pt,
   ...
   \scalebox{0.75}{ \ShadeArea{\x^3-3*\x}{-2.5}{1.7}}
        \end{itemize}
        \item Die Werte unterhalb der $x-$Achse werden dabei negativ gewertet
        \item Die Werte oberhalb der $x-$Achse werden dabei positiv gewertet
\item Für eine gegebene Funktion $f$ mit fester unterer Grenze $a$ definiert $I_a(x) = \integral[limits-mode=limits,limits={a, x}, variable={t}]{f(t)}$ 
die Integralfunktion.
\begin{itemize}
    \item $I_a(x)$ misst die „wachsende Fläche“ von $a$ bis zu einer beliebigen oberen Grenze $x$.  
\item Die Integralfunktion ist damit eine Stammfunktion von $f$.
\end{itemize}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},
  width=9cm,   % feste Breite
  height=8cm,   % feste Höhe
  scale only axis,
  axis lines=middle,
  xmin=-3.3, xmax=3.3,
  ymin=-6.3, ymax=6.3,
  ...
  legend style={font=\small,at={(0.02,0.98)},fill=white,draw=none},
  legend pos=outer north east,
  clip=true
]
  % f(x) = x^3 - 3x
  \addplot[thick, blue, domain = -3:3] {x^3 - 3*x};
  \addlegendentry{$f(x)=x^{3}-3x$}

  % F(x) = 1/4 x^4 - 3/2 x^2 + 1.25  (Stammfunktion von f, Nullstelle bei x=1)
  \addplot[thick, red, domain = -4:4] {0.25*x^4 - 1.5*x^2 + 1.25};
  \addlegendentry{$I_1(x)= \integral[limits-mode=limits,limits={1, x}, variable={t}]{f(t)} =\tfrac14 x^{4}-\tfrac32 x^{2}+\tfrac54$}
\end{axis}
\end{tikzpicture}

\item Hauptsatz der Differential- und Integralfunktion
\begin{itemize}
    \item Das bestimmte Integral $\integral[limits-mode=limits,limits={a, b}]{f(x)}$  wird damit durch die Integralfunktion berechnet: $\integral[limits-mode=limits,limits={a, b}]{f(x)}=F(b)-F(a)$.
\end{itemize}
\end{itemize}
\end{b8d}
\section{Integrationsregeln}
\begin{defi}{Das unbestimmte Integral}{}\index{Integral!unbestimmtes Integral}\label{InteRegel}
Es sei $f$ eine, auf einem Intervall $I$ definierte Funktion. Dann heißt die Funktion $F$ Stammfunktion von $f$, falls gilt $F'(x) = f(x) $ für alle $x\in I$. Das unbestimmte Integral von $f$ ist dann $\int f(x) dx = F(x) +c$ mit $c\in \mathds{R}$.
\end{defi}
\begin{merke}{Regeln zur Integration von Funktionen}{}\index{Integral!Regeln Integration}
    Sind $G$ und $H$ jeweils Stammfunktionen der Funktionen $g$ und $h$, so gilt für alle $k\in \mathds{R}$:\\
    Ist
    \begin{itemize}
        \item $f: x \mapsto k \cdot g(x) \Rightarrow F: x\mapsto k \cdot G(x)$ 
        \item $f: x\mapsto g(x) + h(x) \Rightarrow f: x\mapsto G(x) + H(x)$
    \end{itemize}
    eine mögliche Stammfunktion von f\\
    Sind Funktionen $f$ und $g$ auf einem Intervall $I$ definiert, so gilt für alle $k\in \mathds{R}$ und alle $a,b \in I$:
    \begin{itemize}
        \item $\integral[limits-mode=limits,limits={a, b}]{k \cdot f(x)} = k \cdot \integral[limits-mode=limits,limits={a, b}]{f(x)}$
        \item $\integral[limits-mode=limits,limits={a, b}]{(f(x) + g(x))} = \integral[limits-mode=limits,limits={a, b}]{f(x)} + \integral[limits-mode=limits,limits={a, b}]{g(x)}$
    \end{itemize}
\end{merke}

\begin{merke}{Spezielle Integrationsregeln}{}\index{Integral!spezielle Integrationsregeln}
    Für die Integration von speziellen Funktionen gelten folgende Regeln.
    \begin{multicols}{2}
        \begin{description}
            \item[] allgemein 
            \item[]  $\integral[limits-mode=limits]{f'(x) \cdot e^{f(x)}}= e^{f(x)} +c$
            \item[]  $\integral[limits-mode=limits]{\dfrac{f'(x)}{f(x)}}= \ln{|f(x)|} +c$ mit $f(x) \neq 0$
            \item[] $\integral[limits-mode=limits]{f(ax+b)}= \dfrac{1}{a}\cdot F(ax+b) +c$
            \item[] Beispiel 
            \item[] $\integral[limits-mode=limits]{2x \cdot e^{x^2}}= e^{x^2} +c$ 
            \item[] $\integral[limits-mode=limits]{\dfrac{2x}{x^2+1}}= \ln{(|x^2+1|)} +c = \ln{(x^2+1)} +c$ da $x^2 + 1 >0$
            \item[] $\integral[limits-mode=limits]{\cos{(2x-5)}} = \dfrac{1}{2} \cdot \sin{(2x-5)} +c $ 
        \end{description}
    \end{multicols}
\end{merke}
\begin{b8d}{Verkette Funktion}{}
    Bei der Integrationsregel $\integral[limits-mode=limits]{f(ax+b)}= \dfrac{1}{a}\cdot F(ax+b) +c$ steht $f(ax + b)$ für eine verkettete Funktion, bei der die innere Funktion eine lineare Funktion ist. Die Regel gilt \textcolor{red}{nur} für diese Art von Funktionen.
\end{b8d}
\begin{bsp}{für die Anwendung der Regeln}{}
    Berechne die folgenden Integrale:
    \begin{multicols}{3}
    \begin{description}
        \item[a)] $\integral{\dfrac{2}{x^2}}$
        \item[b)] $\integral{\dfrac{1}{\sqrt{x}}}$
        \item[c)] $\integral{\dfrac{1}{x}}$
    \end{description}   
    \end{multicols}
    \begin{description}
      \item[a)] $\integral{\dfrac{2}{x^2}} = 2\integral{x^{-2}} = 2 \cdot (-1) \cdot x^{-1} +c = -2\cdot \dfrac{1}{x} +c = \dfrac{-2}{x} +c$ 
      \item[b)] $\integral{\dfrac{1}{\sqrt{x}}} = \integral{x^{-0,5}} = 2\cdot x^{0,5} + c = 2\cdot \sqrt{x} + c$ 
      \item[c)] $\integral{\dfrac{1}{x}} = \ln{|x|} + c$  
    \end{description}
    \end{bsp}
         \begin{b8d}{Stammfunktion der Logarithmusfunktion}{}
   Wir suchen die Stammfunktion von $\ln(x)$.
$$
F(x) = \int \ln(x)\,dx
$$
Da der Logarithmus nicht direkt integrierbar ist, verwenden wir die Regel der \textbf{partiellen Integration}:
$$
...
\bigskip
\textbf{Merksatz:}
\emph{Die Stammfunktion von $\ln(x)$ erhält man durch partielle Integration. \\ 
Es gilt:}
$\int \ln(x)\,dx = x\ln(x) - x + C \hspace{0.5cm} \text{und damit ist }
F(x) = x\cdot\ln(x) - x + C$ 
\end{b8d}
\begin{merke}{Verallgemeinerung}{}
    \[
\int \ln(ax+b)\,dx
= \frac{ax+b}{a}\ln(\!\bigl|ax+b\bigr|)
-\frac{ax+b}{a}
+ C
\qquad (a\neq 0)
\]
Der Logarithmus ist nur definiert für
\[
ax+b > 0.
\]
In Stammfunktionen schreibt man deshalb meist \(\ln|ax+b|\).
\end{merke}
\begin{bsp}{Anwendung}{}
   Für \(a=2\) und \(b=3\) erhält man:
\[
\int \ln(2x+3)\,dx
=
\frac{2x+3}{2}\ln\!\bigl|2x+3\bigr|
-\frac{2x+3}{2}
+ C
\] 
\end{bsp}

\section{Flächenberechnungen mit dem Integral}
\subsection{Fläche zwischen Kurve und x-Achse}
\begin{defi}{Flächeninhalt}{}\index{Integral!Flächenberechnung}
    bei der Berechnung des \textbf{Inhalts der Fläche zwischen dem Graphen einer Funktion $f$ und der x-Achse} in einem Intervall $I=[a,b]$ geht man wie folgt vor:
    \begin{enumerate}
        \item Bestimmen der Nullstelle von $f$ in $I=[a,b]$.
        \item Berechnen der Integrale über den Teilintervallen.
        \item Addieren der Inhalte der Teilflächen, d.h. der Beträge der einzelnen Integrale.
    \end{enumerate}  
  \begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},
...
% --- Funktion mit zwei Nullstellen bei -2 und 1 -----------------------------
% f(x) = 2.25 - (x+0.5)^2  (Maximum bei x=-0.5, f=2.25)
\addplot[name path=fL, very thick, blue, domain=-3:-2] {2.25 - (x+0.5)^2};
\addplot[name path=fM, very thick, blue, domain=-2:1 ] {2.25 - (x+0.5)^2};
\addplot[name path=fR, very thick, blue, domain= 1:2 ] {2.25 - (x+0.5)^2};

% --- x-Achsen-Segmente exakt für die Füllintervalle ------------------------
\path[name path=axisL] (axis cs:-3,0) -- (axis cs:-2,0);
\path[name path=axisM] (axis cs:-2,0) -- (axis cs: 1,0);
\path[name path=axisR] (axis cs: 1,0) -- (axis cs: 2,0);

% --- Flächenfärbung NUR im Intervall [-3,2] --------------------------------
\addplot[fill=red!60,   fill opacity=0.25, draw=none] fill between[of=fL and axisL]; % [-3,-2]
\addplot[fill=green!60, fill opacity=0.25, draw=none] fill between[of=fM and axisM]; % [-2,1]
\addplot[fill=red!60,   fill opacity=0.25, draw=none] fill between[of=fR and axisR]; % [1,2]

% --- Senkrechte Grenzlinien des Integrals a=-3, b=2 -------------------------
\draw[densely dashed, gray] (axis cs:-3,-5) -- (axis cs:-3,3);
\draw[densely dashed, gray] (axis cs: 2,-5) -- (axis cs: 2,3);
\node[below] at (axis cs:-3,1) {$a=-3$};
\node[below] at (axis cs: 2,1) {$b=2$};

% --- Optional: Nullstellen markieren (zur Vorzeichen-Orientierung) ----------
\draw[densely dashed, gray!70] (axis cs:-2,-3) -- (axis cs:-2,3);
\draw[densely dashed, gray!70] (axis cs: 1,-3) -- (axis cs: 1,3);
\addplot[only marks, mark=*, mark size=1.8pt] coordinates {(-2,0) (1,0)};

% --- Label der Teilflächen ---------------------------------------------------
\node at (axis cs:-2.6,-1.1) {$A_1$};
\node at (axis cs:-0.5, 1.2) {$A_2$};
\node at (axis cs: 1.5,-1.1) {$A_3$};

\end{axis}
\end{tikzpicture}       
    \end{center}
    Die Stellen $x_1 = - 2$ und $x_2 = 1$ sind die Nullstellen des Graphen $G_f$ von f. Dadurch muss für die Bestimmung der Fläche das Integral in aufgeteilt werden.\\[0.25cm]
    $ A_G =  \left\lvert\integral[limits-mode=limits,limits={-3, -2}]{f(x)} \right\rvert + \integral[limits-mode=limits,limits={-2, 1}]{f(x)} + \left\lvert\integral[limits-mode=limits,limits={1, 2}]{f(x)}\right\rvert =\left\lvert A_1\right\rvert + A_2 + \left\lvert A_3\right\rvert $
\end{defi}
\begin{b8d}{Definitionslücke}{}\index{Integral!Definitionslücke}
Bei der Berechnung des bestimmten Integrals bzw. der Flächenbilanz muss auf die Definitionslücken geachtet werden. An den Definitionslücken ist jede Funktion \textbf{\textcolor{red}{nicht integrierbar}}. Das bedeutet, dass \textbf{\textcolor{blue}{niemals}} über die Definitionslücke integriert werden darf.
\end{b8d}
\begin{bsp}{Die Funktion $f(x) = \dfrac{1}{x}$ }{}
\begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},
  width=12cm,   % feste Breite
  height=8cm,   % feste Höhe
  scale only axis,
  axis lines=middle,
  xmin=-3.3, xmax=3.3,
...
% Optionale Hilfslinien für Integrationsgrenzen
\draw[very thick,dashed, gray] (axis cs:-2,0) -- (axis cs:-2,{1/(-2)});
\draw[very thick, dashed, gray] (axis cs:2,0) -- (axis cs:2,{1/(2)});

\end{axis}
\end{tikzpicture}
\end{center}
 Das Integral $\integral[limits-mode=limits,limits={-2, 2}]{\dfrac{1}{x}}$ der Funktion f mit $f(x) = \dfrac{1}{x}$ und\\ $\mathds{D}_f= \mathds{R}\setminus\{0\}$ ist nicht einfach berechenbar da im Intervall die Definitionslücke $x=0$ liegt. Es ergibt sich für die einzelnen Integrale: 
 $$\lim\limits_{b\longrightarrow 0^-} \integral[limits-mode=limits,limits={-2, b}]{\dfrac{1}{x}} = -\infty$$
 und 
 $$\lim\limits_{a\longrightarrow 0^+} \integral[limits-mode=limits,limits={a, 2}]{\dfrac{1}{x}} = \infty.$$
...
\node[
  ...
  \begin{achtung}[width=5cm,height=2cm]
Definitionslücke!
  \end{achtung}
};
\end{tikzpicture}
\subsection{Fläche zwischen Kurven}
\begin{merke}{Fläche zwischen Kurven}{}\index{Integral!Fläche zwischen Kurven}
    Der Inhalt A einer \textbf{\textcolor{blue}{Fläche, die im Intervall $I=[a,b]$ von dem Graphen zweier Funktionen $f$ und $g$ begrenzt wird}}, wobei $f(x)\geq g(x)$ bzw. $g(x)\geq f(x)$ gelten muss, berechnet sich mit: $$A= \left\lvert\integral[limits-mode=limits,limits={a, b}]{(f(x) - g(x))} \right\rvert$$
 \end{merke}
  \begin{b8d}{Vorgehen für Flächen zwischen Kurven}{}
Bei der Berechnung des \textbf{\textcolor{blue}{Inhalts der Fläche zwischen den Graphen zweier Funktionen $f$ und $g$}} im Intervall $I=[a,b]$ geht man wie folgt vor:
\begin{enumerate}
    \item Bestimmen aller Schnittstellen $x_i$ der beiden Funktionen in $I=[a,b]$
    \item Berechnen und Addieren der Inhalte der Teilflächen
\end{enumerate}
...
% Konstruktion mit 3 Schnittpunkten: f - g = (x+2)*x*(x-2)/8
\addplot[name path=f, very thick, blue, domain=-3.5:3.5]
  {0.2*x^2 - 1 + (x^3 - 4*x)/8};

\addplot[name path=g, very thick, black!70, domain=-3.5:3.5]
  {0.2*x^2 - 1};

% --- Unsichtbare Teilpfade zum exakten Füllen (senkrechte Ränder) -----------
\addplot[name path=fAB, draw=none, domain=-2:0] {0.2*x^2 - 1 + (x^3 - 4*x)/8};
\addplot[name path=gAB, draw=none, domain=-2:0] {0.2*x^2 - 1};

\addplot[name path=fBC, draw=none, domain=0:2]  {0.2*x^2 - 1 + (x^3 - 4*x)/8};
\addplot[name path=gBC, draw=none, domain=0:2]  {0.2*x^2 - 1};

% --- Flächenfärbung NUR zwischen dem linken und rechten Schnittpunkt --------
% [-2,0]: f >= g  → positiv (grün)
\addplot[fill=green!60, fill opacity=0.25, draw=none]
  fill between[of=fAB and gAB];

% [0,2]:  f <  g  → negativ (rot)
\addplot[fill=red!60,   fill opacity=0.25, draw=none]
  fill between[of=fBC and gBC];

% --- Schnittpunkte und senkrechte Hilfslinien -------------------------------
\draw[densely dashed, gray, thick] (axis cs:-2,-3) -- (axis cs:-2,3);
\draw[densely dashed, gray, thick] (axis cs: 0,-3) -- (axis cs: 0,3);
\draw[densely dashed, gray, thick] (axis cs: 2,-3) -- (axis cs: 2,3);

% --- Labels der Funktionen --------------------------------------------------
\node at (axis cs:-3,  1.5) {$f(x)$};
\node[blue] at (axis cs: 2.7,2.7) {$g(x)$};
\end{axis}
\end{tikzpicture}
\end{center}
$$A_g = \left\lvert\integral[limits-mode=limits,limits={-2, 0}]{(f(x) - g(x))}   \right\rvert + \left\lvert\integral[limits-mode=limits,limits={0, 2}]{(f(x) - g(x))}   \right\rvert$$
 \end{b8d}
\begin{tikzpicture}[remember picture, overlay]
% --- Der Hinweis-Kasten ---
\node[
  ...
  transform shape,
  inner sep=0pt
] at ([xshift=-0.5cm,yshift=12cm]current page.south east) {%
  \begin{achtung}[width=5cm,height=4cm]
Bei der Berechnung der Fläche ist es egal, welche Funktion man von welcher abzieht. Durch den Betrag bekommt man immer die richtige Fläche.
  \end{achtung}
};
\end{tikzpicture}

 \section{Das uneigentliche Integral}
In den vorangegangenen Abschnitten wurden nur Integrale in abgeschlossenen Intervallen betrachten. Es wurden Flächenbilanzen bzw. Flächen unter Kurven in begrenzten Bereichen bestimmt. Im folgenden wird mithilfe der Integration untersucht, ob auch \glqq ins Unendlich reichende \grqq Flächen einen endlichen Flächeninhalt haben können. Hierfür wird aus
$$
\highlightmath{\integral[limits-mode=limits,limits={1, \infty}]{\dfrac{2}{x^2}}=\lim\limits_{a\longrightarrow \infty} \integral[limits-mode=limits,limits={1, a}]{\dfrac{2}{x^2}}}$$
\begin{bsp}{Berechnung uneigentliches Integral}{}
\begin{center}
\begin{tikzpicture}
\begin{axis}[ticklabel style={fill=none},   every tick label/.append style={fill=none},
  width=10cm,   % feste Breite
  height=8cm,   % feste Höhe
  scale only axis,
  axis lines=middle,
  xmin=-0.3, xmax=7.3,
...
% Kurve
\addplot[red, thick, domain=0.1:7.2] {2/x^2};
...
% Optionale Hilfslinien für Integrationsgrenzen
\draw[very thick, dashed, gray] (axis cs:1,0) -- (axis cs:1,{2});

\draw[very thick, dashed, red] (axis cs:4,0) -- (axis cs:4,{1/(8)});

\node[below,yshift=-6pt] at (axis cs:4.5,0)
    {$\textcolor{red}{\xlongrightarrow{\;a\;} \infty}$};
\end{axis}
\end{tikzpicture}
\end{center}
Berechnung des uneigentlichen Integrals $$\integral[limits-mode=limits,limits={1, \infty}]{\dfrac{2}{x^2}} = \lim \limits_{a \longrightarrow\infty}\integral[limits-mode=limits,limits={1, a}]{\dfrac{2}{x^2}}= \lim \limits_{a \longrightarrow\infty} \left[-2x^{-1}\right]_1^a $$ daraus folgt für die Differenz
$$\lim \limits_{a \longrightarrow\infty} -\dfrac{2}{a} - \left(-\dfrac{2}{1}\right) = \lim \limits_{a \longrightarrow\infty} -\dfrac{2}{a} +2 = 0^-+2 = 2. $$
Damit hat die ins unendliche reichende Fläche einen endlichen Flächeninhalt. 
\end{bsp}
\begin{defi}{Das uneigentliche Integral}{}
    Existiert für eine im Intervall $I= \left[a; \infty \right[$ bzw. $I = \left]a;b \right]$ definierte Funktion $f$ $$\lim\limits_{z\longrightarrow \infty} \integral[limits-mode=limits,limits={a, z}]{f(x)} \hspace{0.5cm} \text{bzw.} \hspace{0.5cm} \lim\limits_{z\longrightarrow a} \integral[limits-mode=limits,limits={z, b}]{f(x)}$$ 
    so nennt man diesen Grenzwert das \textbf{\textcolor{red}{uneigentliche Integral}} über dem betreffenden Intervall.
\end{defi}
\begin{bsp}{Berechnung uneigentliches Integral}{}
\begin{center}
\begin{tikzpicture}
...
% Kurve
\addplot[red, thick, domain=0.1:3.2] {2/x^2};
...
\draw[very thick, dashed, red] (axis cs:0.5,0) -- (axis cs:0.5,8);

\node[below,yshift=-1pt] at (axis cs:0.25,0)
    {$\textcolor{red}{0 \xleftarrow
{\;z\;}}$};
\end{axis}
\end{tikzpicture}
\end{center}
Berechnung des uneigentlichen Integrals $$\integral[limits-mode=limits,limits={0, 1}]{\dfrac{2}{x^2}} = \lim \limits_{z \longrightarrow 0}\integral[limits-mode=limits,limits={z, 1}]{\dfrac{2}{x^2}}= \lim \limits_{z \longrightarrow 0} \left[-2x^{-1}\right]_z^1 $$ daraus folgt für die Differenz
$$
\highlightmath{\integral[limits-mode=limits,limits={1, \infty}]{\dfrac{2}{x^2}}=\lim\limits_{a\longrightarrow \infty} \integral[limits-mode=limits,limits={1, a}]{\dfrac{2}{x^2}}}$$
Damit hat die eng begrenzte Fläche einen unendlichen Flächeninhalt. 

\end{bsp}
\begin{rechnung}{}{}
   Untersuchungen am Beispiel: \\[0.3cm]$f(x)= x+\dfrac{1}{4\sqrt{x}}$ \hspace{0.3cm} mit \hspace{0.3cm} $\mathds{D}= \mathds{R}^+$ \\[0.3cm] $a) A(z) = \lim\limits_{z \longrightarrow 0}\integral[limits-mode=limits,limits={z, 1}]{x+\dfrac{1}{4\sqrt{x}}} = \lim\limits_{z \longrightarrow 0}\left[\dfrac{1}{2}x^2 + \dfrac{1}{2} \sqrt{x}\right]_z^1 = \lim\limits_{z \longrightarrow 0} \left(1-\dfrac{1}{2}z^2 - \dfrac{1}{2}\sqrt{z}\right)= 1$\\[0.3cm] Damit hat die Fläche für $z\longrightarrow 0$ den Wert $1$\\[0.3cm] b) $A(b) = \lim\limits_{b \longrightarrow \infty}\integral[limits-mode=limits,limits={1, b}]{\left(x-(x+\dfrac{1}{4\sqrt{x}})\right)} = \lim\limits_{b \longrightarrow \infty}\integral[limits-mode=limits,limits={1, b}]{\dfrac{1}{4\sqrt{x}}} =\lim\limits_{b \longrightarrow \infty}  \left[\dfrac{1}{2} \sqrt{b}\right]_1^b $  \\[0.3cm] $=\lim\limits_{b \longrightarrow \infty} \dfrac{1}{2} \sqrt{b} -\dfrac{1}{2} = \infty$
\end{rechnung}
\section{Das Integral als Gesamtänderung einer Fläche}
\begin{defi}{Von der Änderungsrate zum Bestand}{}\index{Integral!Änderungsrate}
    Gegeben ist eine in einem Intervall $I$ integrierbare Funktion $f:x\mapsto f(x)$. Beschreibt die Funktion $f$ im Intervall $I$ die lokale bzw. momentane Änderungsrate einer Größe, so entspricht das bestimmte Integral $\integral[limits-mode=limits,limits={a, b}, variable ={t}]{f(t)}$ mit $a,b \in I$ der Gesamtänderung dieser Größe im Intervall $I=[a|b]$.
\end{defi}
...
\begin{multicols}{2}
Ein Rotationskörper\footnote{Alle Bilder aus \cite{klett_lambacher_schweizer_mathe13_bayern_2025} S.42} entsteht, wenn die vom Graphen einer Funktion $f$ über dem Intervall $I=[a;b]$ eingeschlossenen Fläche um die $x-$Achse rotiert. \\
\begin{center}
\includegraphics[width = 0.3\textwidth]{Bilder/Rotationskoerper1.png}    
\end{center}
Die Bestimmung des Volumens bei Rotationskörpern orientiert sich am Vorgehen bei der Definition des Integrals, Flächeninhalte durch Rechtecksummen anzunähern.\\
\begin{center}
\includegraphics[width = 0.4\textwidth]{Bilder/Rotationskoerper2.png}    
\end{center}
Man nähert bei Rotationskörpern das Volumen durch eine Summe von Zylindervolumina an. Es werden Zylinder der Höhe $h = \Delta x$ betrachtet, deren Volumina sich mit der Formel $V_z = \pi r^2 \cdot h$ berechnen lassen. Somit gilt für das Volumen $V_i$ rechts einer Stelle $x_i$ folgender Zusammenhang: $V_i = \pi\cdot (f(x_i))^2 \cdot \Delta x$. Je schmaler die einzelnen Zylinder werden, desto besser wird das Volumen des Rotationskörpers angenähert.  
\begin{center}
\includegraphics[width = 0.3\textwidth]{Bilder/Rotationskoerper3.png}    
\end{center} 
Für eine stetige Funktion $f$ ist das Volumen des Rotationskörpers folglich der Grenzwert der einzelnen Zylindervolumina für $n\rightarrow \infty$ und damit $\Delta x \rightarrow 0$ und somit $$V = \integral[limits-mode=limits,limits={a, b}]{\pi \cdot (f(x))^2} = \pi \cdot \integral[limits-mode=limits,limits={a, b}]{(f(x))^2}$$
\end{multicols}
\newpage
\begin{defi}{Volumen von Rotationskörpern}{}
  Die Funktion $f$ sei über einem Intervall $I$ mit $I=[a;b]$ stetig. Rotiert die Fläche zwischen dem Graphen von $f$ und der $x-$Achse im Intervall $I=[a;b]$ um die $x-$Achse, so entsteht ein \textbf{\textcolor{red}{Rotationskörper}}.\\
  Sein Volumen berechnet man in Anlehnung an die Fläche mit $$V= \pi \integral[limits-mode=limits,limits={a, b}]{(f(x))^2}$$
\end{defi}