...
\end{itemize}
\end{merke*}
Um die Ableitung zu definieren, betrachten wir die Normalparabel mit der Gleichung $$ y=x^2 $$ und darauf einen Punkt $P(1,5|2,25)$. Zur Bestimmung deSekantensteigung bildet man den Differenzenquotienten zwischen dem Punkt $P$ und einem weiteren beliebigen Punkt $Q(x|x^2)$. 
...
\begin{axis}[xmin= -2, xmax = 3.5, ymin= -1.5, ymax= 3.5,
        ...
        \draw[color= red] (0.8,2.5) node[right] {$P$};
          ...
          \draw (1,0) coordinate (a) -- (0.33,0) coordinate (b) -- (1,2) coordinate (c)  pic[ draw = blue, fill = red!15!white, ->, angle eccentricity =1.2, angle radius = 1cm] {angle = a--b--c};
                  \draw[color= black] (1,0.8) node[right] {$\alpha$};
\end{axis}
\end{tikzpicture}
\end{center}
\end{bem*}
...
\begin{enumerate}
    ...
    \item Berehnung des Steigungswinkels der Tangente an dieser Stelle am Graphen
\end{enumerate}
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
Berechnung des Steigungswinkels der Tangente:
\end{bsp*}
...
\begin{enumerate}
    \item Bildung er ersten Ableitung
...
\begin{defi}{Der Wendepunkt}{}\index{Wendepunkt}\index{Kurvendiskussion!Wendepunkt}
...
\begin{b8d}{Rest der Polynomdivision}{rest}
  Bei der Polynomdivision zur Bestimmung der Nullstellen eines Polynoms treten \textcolor{red}{keine Rest} auf.   
\end{b8d}
\subsection{Polynomdivision bei ganzrationalen Funktionen}\index{Polynomdivision!ganzrationale Funktionen}\label{polynomdivisionBruch}
Die zweite Anwendung der Polynomdivision besteht in der Umwandlung der Bruchform in die Summenform der ganzrationalen Funktionen. Hioerbei wird die Polynomdivision durchgeführt indem man den Zähler der Funktion durch seinen Nenner dividiert.
\printindex
