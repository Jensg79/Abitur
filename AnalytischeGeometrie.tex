...
Dieser Vektor repräsentiert genau einen Pfeil der im Ursprung des Koordinatensystems beginnt und an den Koordinaten des Punktes $P$ endet.\\ 
...
\node[
  ...
  \begin{achtung}[width=5.5cm,height=4cm]
Spitze minus Hacke!\\ Es besteht ein Unterschied zwischen einem Ortsvektor und einem Verbindungsvektor!
  \end{achtung}
};
\end{tikzpicture}
...
Alle Punkte, die von einem gegeben Punkt $M(m_1|m_2|m_3)$ die gleiche Entfernung $r$ haben, liegen auf einer Kugel um den Mittelpunkt $M$. Die Koordinaten aller dieser Punkte $X(x_1|x_2|x_3)$ genügen folgender Gleichung: $$(x_1-m_1)^2 + (x_2 -m_2)^2 + (x_3 -m_3)^2 = r^2$$
\end{b8d}
...
\node[
  ...
  transform shape,
  inner sep=0pt
] at ([xshift=-0.5cm,yshift=-17cm]current page.north east) {%
  \begin{achtung}[width=4.5cm,height=3.5cm]
skalare Multiplikation: Vektor\\
Skalarprodukt: reelle Zahl
  \end{achtung}
};
\end{tikzpicture}
...
\end{merke}
\begin{satz}{Geometrische Deutung}{}\index{Vektoren!Skalarprodukt!Geometrische Deutung}
    \begin{itemize}
  \item Für \(0^\circ \le \varphi \le 90^\circ\) ist \(\cos(\varphi)\ge 0\):  
  \(\vv{a}\circ\vv{b}\) ist \textbf{positiv} und entspricht dem Produkt aus \(|\vv{a}|\) und der Projektion von \(\vv{b}\) auf \(\vv{a}\).
  \item Für \(90^\circ < \varphi \le 180^\circ\) ist \(\cos(\varphi)< 0\): 
  \(\vv{a}\circ\vv{b}\) ist \textbf{negativ}.
  \item Für \(\varphi=90^\circ\): \(\cos(\varphi)=0\Rightarrow \vv{a}\circ\vv{b} = 0\).
\end{itemize}
\begin{center}
\begin{tikzpicture}[scale=1.0, line cap=round, line join=round, >={Latex[length=2.4mm]}]

% ---------- Links: 0° <= phi <= 90° ----------
\begin{scope}
  \node[anchor=west] at (-0.2,2.55) {\small \(0^\circ \le \varphi \le 90^\circ\) \ (\(\cos\varphi\ge 0\))};

  % Punkte definieren
  \coordinate (O) at (0,0);
  \coordinate (Aax) at (3.6,0);      % Richtung von a
  \coordinate (Bvec) at (2.2,1.9);   % Spitze von b
  \coordinate (P) at (2.2,0);        % Projektion auf a

  % a und b
  \draw[blue!70, very thick, ->] (O) -- (Aax) node[below] {\(\vec a\)};
  \draw[blue!70, very thick, ->] (O) -- (Bvec) node[above left=-1pt] {\(\vec b\)};

  % Projektion (Lot)
  \draw[red!70, dashed, thick] (Bvec) -- (P);
  \fill[red!70] (P) circle (1.2pt);

  % projizierte Strecke
  \draw[green!60!black, thick] (O) -- (P);
  \node[green!60!black] at ($(O)!0.5!(P)+(0,-0.35)$) {\(|\vv{b}|\cos\varphi\)};

  % Winkel phi am Ursprung zwischen a-Richtung und b
\pic[draw=green!60!black, thick, angle radius=10pt] {angle=Aax--O--Bvec};
% Label separat (Position ggf. minimal anpassen)
\node[green!60!black] at (0.55,0.25) {$\varphi$};
\end{scope}

% ---------- Rechts: 90° < phi <= 180° ----------
\begin{scope}[xshift=8.2cm]
  \node[anchor=west] at (-0.2,2.55) {\small \(90^\circ < \varphi \le 180^\circ\) \ (\(\cos\varphi< 0\))};

  % Punkte definieren
  \coordinate (O2) at (0,0);
  \coordinate (Aax2) at (3.6,0);       % Richtung von a
  \coordinate (Bvec2) at (-1.6,1.7);   % Spitze von b (stumpfer Winkel)
  \coordinate (P2) at (-1.6,0);        % Projektion auf a

  % a und b
  \draw[blue!70, very thick, ->] (O2) -- (Aax2) node[below] {\(\vec a\)};
  \draw[blue!70, very thick, ->] (O2) -- (Bvec2) node[above left=-1pt] {\(\vec b\)};

  % Projektion (Lot)
  \draw[red!70, dashed, thick] (Bvec2) -- (P2);
  \fill[red!70] (P2) circle (1.2pt);

  % projizierte Strecke (gerichtet nach links -> negativ)
  \draw[green!60!black, thick] (O2) -- (P2);
  \node[green!60!black] at ($(O2)!0.5!(P2)+(0,-0.35)$) {\(|\vv{b}|\cos\varphi\) (neg.)};

  % Winkel phi
\pic[draw=green!60!black, thick, angle radius=10pt] {angle=Aax2--O2--Bvec2};
\node[green!60!black] at (0.55,0.25) {$\varphi$};
\end{scope}
...
Die Vektoren $\vv{a} = \begin{pmatrix} 1\\2\\3
...
\end{pmatrix}$ und $\vv{c}= \begin{pmatrix}
   3\\1\\2
\end{pmatrix}$ wird eine dreiseitige Pyramide aufgespannt. 
\[
\det(\vv{a}, \vv{b}, \vv{c})
%
=\begin{matrix}\begin{tikzpicture}
...
\node[
  ...
  \begin{achtung}[width=4.5cm,height=2.5cm]
Der Vektor $\vv{X}$ steht für einen bel. Punkt auf der Geraden.
  \end{achtung}
};
\end{tikzpicture}
