...
\begin{itemize}
    ...
    \item Scheitelpunktform: Hierbei ist $SP(x_s|y_s)$ der Scheitelpunkt des Graphen der quadratischen Funktion $$f(x) = a\cdot (x-x_s)^2 +y_s$$ diese Darstellung gilt auch für die kubische Funktion $$f(x) = a\cdot (x-x_{\footnotesize{W}})^3 +y_{\footnotesize{W}}$$ 
...
\begin{center}
    ...
    \end{tikzpicture}  
\end{center}
\section{Übersicht von Ableitungsregeln mit Beispielen} \index{Ableitung! Übersicht} \label{AbleitungAllgemein}
...
\label{sec:L_Hospital} % Label für den Verweis aus dem Inhaltsverzeichnis
\begin{merke}{Erläuterung zur Regel von L'Hospital}{}
Erreicht der Grenzwert einer Funktion \(f\) einen unbestimmten Ausdruck der Form \(\dfrac{0}{0}\), \(\dfrac{\infty}{\infty}\) oder \(0 \cdot \infty\) ist dieser Grenzwert in vielen Fällen trotzdem berechenbar.\\
Die Regel von L'Hospital lautet in diesen Fällen: \\
    Existiert der Grenzwert \[\lim\limits _{x\longrightarrow x_0} f(x) = \lim\limits _{x\longrightarrow x_0} \dfrac{z'(x)}{n'(x)}\] so ist dieser sogleich der Grenzwert des Terms \[f(x) = \dfrac{z(x)}{n(x)}\] wobei hier \(z'(x)\) und \(n'(x)\) jeweils die 1. Ableitungen der Funktionen \(z(x)\) und \(n(x)\) sind.
...
Bei der Anwendung der Regel von L'Hospital besteht kein Zusammenhang mit der Quotientenregel der Ableitung.
  \end{achtung}
};
\end{tikzpicture}
\section{Extremwertprobleme}\index{Extremwertprobleme} 
   In vielen Anwendungssituationen sind Extremwerte einer Funktion gesucht, wobei erst eine Modellfunktion, zur Beschreibung des Problems, bestimmt werden muss. Zum lösen von Extremwertproblemen ist ein einfaches systematisches Vorgehen sehr sinnvoll.  
   \begin{defi}{Extremwertprobleme}{}
Vorgehen beim Lösen von \textbf{\textcolor{red}{Extremwertproblemen}}:
\begin{enumerate}
    \item Beschreiben der zu maximierenden bzw. minimierenden Größe durch einen Term. Dieser Term kann mehrere Variablen enthalten.
    \item Formulierung von Nebenbedingungen zum Beschreiben der Abhängigkeiten zwischen den Variablen.
    \item Bestimmen der \textbf{\textcolor{red}{Zielfunktion}}, diese ist nur noch von einer variablen abhängig.
    \item Untersuchen der Zielfunktion auf \textbf{\textcolor{red}{Extremwerte}} und \textbf{\textcolor{red}{Formulierung}} des Ergebnisses. Hierfür sind gegebenfalls auch die Randextrema zu berücksichtigen.
\end{enumerate}
   \end{defi}
   \begin{merke}{Randextrema}{}
    Ein \textbf{Randextremum} ist ein \textbf{Maximum} oder \textbf{Minimum} einer Funktion, das \textbf{an einem Randpunkt des betrachteten Definitionsbereichs} liegt.\\
    Bei Extremwertaufgaben auf einem abgeschlossenen Intervall \( [a,b] \) m\"ussen immer
\begin{itemize}[leftmargin=1.5em]
    \item die kritischen Stellen im Inneren des Intervalls und
    \item die Randpunkte \(a\) und \(b\)
\end{itemize}
untersucht werden.   
   \end{merke}
   \begin{bsp}{}{}
...
f(x)=x^2 \quad \text{mit} \quad x\in[1,3]\). Innerhalb des Intervalls existieren keine Extrempunkte. Allerdings gilt für die Stellen am Rand des Intervalls:
\(f(1)=1, \quad f(3)=9\). Da \(x=1\) und \(x=3\) Randpunkte des Intervalls sind, liegen hier Randextrema vor:
\begin{itemize}[leftmargin=1.5em]
    \item \(f(1)=1\) ist das \textbf{Minimum},
    \item \(f(3)=9\) ist das \textbf{Maximum}.
\end{itemize}
\begin{center}
\begin{tikzpicture}[scale=0.5]
    \draw[->] (0,0) -- (4.2,0) node[right] {\(x\)};
    \draw[->] (0,0) -- (0,10.5) node[above] {\(f(x)\)};    \draw[domain=1:3,smooth,variable=\x,thick] plot ({\x},{\x*\x});

    \filldraw (1,1) circle (2pt) node[below left] {\((1,1)\)};
    \filldraw (3,9) circle (2pt) node[right] {\((3,9)\)};
...
\end{multicols}
   ...
   \begin{b8d}{Vorgehen bei Extremwertproblemen}{}
Bei einer Untersuchung auf absolute Extremwerte im Intervall \( [a,b] \) gilt:
\begin{enumerate}[leftmargin=1.5em]
    \item Ableitung bilden,
    \item kritische Stellen im Inneren bestimmen,
    \item Randpunkte einsetzen,
    \item alle Funktionswerte vergleichen.
\end{enumerate}
   \end{b8d}
   \begin{tikzpicture}[remember picture, overlay]
...
\node[
  ...
  transform shape,
  inner sep=0pt
] at ([xshift=-3.5cm,yshift=-14cm]current page.north east) {%
  \begin{achtung}[width=5cm,height=3cm]
Immer an die Randextrema denken!
  \end{achtung}
};
\end{tikzpicture}
